...
def filter_amigo(amigo_file, filter_file, make_dir):
	lines = []
	amigo_apps = os.popen("make -C "+ make_dir + " amigo-apps-included").read()
	with open(amigo_file, 'r') as mf:
		lines = mf.readlines()

	with open(filter_file, 'w') as ff:
		for line in lines:
			if line.split():
				if (line.split()[0].split('"')[1] in amigo_apps):
					ff.write(line)
...
def main(args):
	...
	filter_amigo(args[1], filtered, args[2])
