...
if [ $# -ne 3 ];then
	echo "Usage: patchamigo.sh old_smali_dir new_smali_dir dst_smali_dir"
fi
...
function apply_amigo_patch() {
	echo "<<< entry apply_amigo_patch()"
...
#apply_amigo_patch android.policy.jar.out
apply_amigo_patch services.jar.out
apply_amigo_patch framework.jar.out

echo
echo
echo ">>> patch amigo into target framework is done. Please look at $reject_dir to resolve any conflicts!"
